...
to go
  if all? turtles [symptoms?]
    [ stop ]

  if all? turtles [not symptoms? and not contaminated?]
    [ stop ]

  ask turtles
  [
     set symptoms-show-time symptoms-show-time + 1
     if symptoms-show-time >= symptoms-show-max-time
       [ set symptoms-show-time 0 ]
  ]
  spread
  symptoms-appear
  recovery-checks
...
to become-contaminated  ;; turtle procedure
  set symptoms? false
...
"" ""
PENS
"No symptoms" 1.0 0 -13345367 true "" "plot (count turtles with [contaminated?]) / (count turtles) * 100"
"Symptoms" 1.0 0 -2674135 true "" "plot (count turtles with [symptoms?]) / (count turtles) * 100"
